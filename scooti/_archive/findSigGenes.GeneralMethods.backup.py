...
class findSigGenes:
    ...
    def get_compared_genes(self, alpha=0.95, split_str='_', prefix_define='', method='CI', std_num=2, correction=False, save_files=False):
        ...
        if method=='CI':
            ...
            # compute bounds from reference cells only on raw expression (no batch correction, no z-score)
            ref_df = genedf[ref_cells]
            ub = ref_df.mean(axis=0) + std_num * ref_df.std(axis=0)
            lb = ref_df.mean(axis=0) - std_num * ref_df.std(axis=0)
            print('shape of limits:', ub.shape)
            # get up- or down-regulated genes for each exp cell using strict bounds
