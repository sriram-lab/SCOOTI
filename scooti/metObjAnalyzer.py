...
class metObjAnalyzer:
    ...
    def get_coef(self, metType_cluster=False):
        ...
        if self.label_func is None:
            col_to_group = {}
            for k, dfk in coefs.items():
                for c in dfk.columns:
                    col_to_group[c] = k
            labels = pd.Series([col_to_group.get(c, '') for c in coef_sel.columns], index=coef_sel.columns)
        else:
    ...
    def coefAnalysis(
        ...
        if distance:
            try:
                # distance to biomass
                # get biomass coefficients using configured GEM_path when available
                gem_path = self.GEM_path if isinstance(self.GEM_path, str) and len(self.GEM_path) > 0 else '/home/daweilin/StemCell/cancer_model.mat'
                ...
                print('[WARN] Skipping distance-to-biomass due to error:', e)
