function multiObj_CBM(config)
  ...
  % Resolve this module's directory to build robust file paths
...
if isempty(config.obj_candidate_list_file)
    % Default to GEMs list relative to this file's directory
    config.obj_candidate_list_file = fullfile(this_dir, 'GEMs', 'obj52_metabolites_shen2019.csv');
end
config.obj_candidates = readtable(config.obj_candidate_list_file);

if isempty(config.input_obj_tb)
    % Default buffer objective coefficients relative to this module
    config.input_obj_tb = readtable(fullfile(this_dir, 'bufferFiles', 'buffer_objective_coefficients.csv'));
...
if config.constraint == 1
    ...
    % Default constraint buffer relative to this module
    config.data_series = {fullfile(this_dir, 'bufferFiles', 'buffer_constraint.xlsx')};
...
for data_idx = 1:length(config.data_series)
    ...
    if ~config.input_objective_weights & strcmp(config.prefix_name, 'model'),
      % Iterate over all candidate metabolites. select_objectives prepends a
      % leading '' to the list, so the valid indices are 2..(N+1). The prior
