...
def unconstrained_models(root_path, norm=False, return_variables=True, medium='DMEMF12', biomass_reaction_name='biomass_objective', file_suffix='_fluxes.csv.gz'):
    ...
    for file in flux_files:
        if '_metadata' in file:
            try:
                ...
                if medium_from_model!=medium:
                    medium_mismatch += 1
                if medium_from_model==medium and colname!='':
                    matched_medium += 1
                    prefix = file.split('_metadata')[0]
                    # Preferred candidate based on configured suffix
                    cand = os.path.join(root_path, f"{prefix}{file_suffix}")
                    alt = None
                    if not os.path.isfile(cand):
                        # Fallback: try swapping .csv.gz <-> .csv
                        if cand.endswith('.csv.gz'):
                            alt = cand[:-3]
                            if os.path.splitext(cand)[1] == '.gz':
                                missing_gz.append(cand)
                        elif cand.endswith('.csv'):
                            alt = cand + '.gz'
                            missing_csv.append(cand)
                    if alt and os.path.isfile(alt):
                        cand = alt
                    if not os.path.isfile(cand):
                        print('No file', cand)
                        missing_both.append(cand)
                        continue
                    df = pd.read_csv(cand)
                    row = df.iloc[:,0]#['rxns']
                ...
                # Keep legacy message but prefer suffix-aware path when possible
                try:
                    pf = file.split('_metadata')[0]
                    import os as _os
                    print('No file', _os.path.join(root_path, f"{pf}{file_suffix}"))
                except Exception:
    ...
    if len(df_collect) == 0:
        ...
        reasons = []
        if total_meta == 0:
            reasons.append(f"No metadata files (*_metadata.json) found in: {root_path}")
        if matched_medium == 0 and medium_mismatch > 0:
            reasons.append(f"No models matched medium='{medium}' (found {medium_mismatch} with non-matching medium)")
        if missing_gz and file_suffix.endswith('.csv.gz'):
            reasons.append("No *_fluxes.csv.gz files found for matching metadata")
        if missing_csv and file_suffix.endswith('.csv'):
            reasons.append("No *_fluxes.csv files found for matching metadata")
        if missing_both and not reasons:
            reasons.append("No *_fluxes.csv(.gz) files found for matching metadata")
        details = '; '.join(reasons) if reasons else f"No flux files found in: {root_path}"
        raise ValueError(f"No objects to concatenate. {details}. Check medium and fileSuffix settings.")
...
def getRxnCoefficients(
    ...
    if model_name=='Recon3D':
        ...
        # Prefer the CSV biomass coefficient list; fall back to COBRA only if CSV is unavailable
        candidates = []
        if biomass_csv_path:
            candidates.append(biomass_csv_path)
        # alongside model_path
        try:
            model_dir = os.path.dirname(os.path.abspath(model_path))
            candidates.append(os.path.join(model_dir, 'recon1_biomass_coef.csv'))
        except Exception:
            pass
        # known repo location
        try:
        ...
        if csv_used:
            ...
            tmp.columns = cols
            if not {'Objective','coefficient'}.issubset(set(tmp.columns)):
                # attempt to coerce first two columns
                tmp = tmp.rename(columns={tmp.columns[0]:'Objective', tmp.columns[1]:'coefficient'})
            biomass_coef_df = tmp[['Objective','coefficient']].copy()
            # strip compartments if present and aggregate duplicates
            def _strip_comp(s: str) -> str:
                s = str(s)
                base = s.split('[')[0]
                if base.endswith('_c') or base.endswith('_m') or base.endswith('_n') or base.endswith('_r') or base.endswith('_g') or base.endswith('_l') or base.endswith('_x') or base.endswith('_p') or base.endswith('_e'):
                    base = base[:-2]
                return base
            biomass_coef_df['Objective'] = biomass_coef_df['Objective'].map(_strip_comp)
            biomass_coef_df = biomass_coef_df[biomass_coef_df['Objective']!='h2o']
            biomass_coef_df = biomass_coef_df.groupby('Objective', as_index=True)['coefficient'].sum().to_frame()
        else:
            # fallback to COBRA
            try:
                model = cobra.io.load_matlab_model(model_path)
                ...
                print('[ERROR] Could not obtain biomass coefficients via CSV or COBRA:', e)
                raise
...
def coef_distance_to_biomassObj(
    ...
    # Ensure labels align positionally with cells to avoid NaN and palette None
    try:
        corrs['cellTypes'] = np.array(labels)
    except Exception:
        corrs['cellTypes'] = list(labels)
    out_copy = corrs.copy()
    if zscore:
    ...
    if histplot==True:
        # ensure stable palette mapping
        cats = list(pd.unique(corrs['cellTypes']))
        pal = {k: c for k, c in zip(cats, sns.color_palette('Pastel2', n_colors=len(cats)))}
        # creating a figure composed of two matplotlib.Axes objects (ax_box and ax_hist)
        ...
        sns.histplot(
                ...
                palette=pal, common_norm=False, bins=100, hue_order=boxplot_order or cats,
...
def metabolites_dist_to_biomassObj(
    ...
    if histplot==True:
        ...
        sns.histplot(
                data=corrs, x='Distances', hue='cellTypes', stat='density', kde=True,
                palette=pal, common_norm=False, bins=100, log_scale=False, element='step', ax=ax_hist,
                )
        # plot zero lines
        ax_box.axvline(x=0, linestyle='--', color='grey')
        #ax_hist.axvline(x=0, linestyle='--', color='grey')
...
def coef_distance_to_biomassFlux(
        ...
        if histplot==True:
            ...
            # plot each data point
            g = sns.stripplot(y='cellTypes', x='Distances', s=10, palette=pal,
                    data=corrs, hue='cellTypes', alpha=.5, ax=ax_box, zorder=1, hue_order=cats)
            gp = sns.pointplot(y="cellTypes", x='Distances',
                          data=corrs, dodge=False,#.8 - .8 / 3,
                          join=False, hue='cellTypes',palette="Dark2",
                          markers="x", scale=1.8, ci=None, ax=ax_box, hue_order=cats)
            ax_box.get_legend().remove()
            # Remove x axis name for the boxplot
            ylabel_colors = []
            for ct in corrs['cellTypes'].unique():
...
class clustering_func:
    ...
    def dimension_reduction_func(pca_df, para, func='PCA', seed=8, transparent=0.8):
        ...
        if func=='UMAP':
            ...
            # Safe checks: cap params by sample size to avoid ARPACK eigsh(k>=N)
            # Accept para as [n_neighbors, n_components]
            try:
                requested_neighbors = int(para[0])
            except Exception:
                requested_neighbors = 30
            try:
                requested_components = int(para[1])
            except Exception:
                requested_components = 2

            # Fall back to N-1 components to satisfy eigsh requirement k < N (spectral init)
            safe_neighbors = max(2, min(requested_neighbors, n_samples - 1))
            safe_max_components = max(2, n_samples - 1)
            n_components = max(2, min(requested_components, safe_max_components))
            # If reaching the boundary (N-1), switch to random init to avoid eigsh k>=N in some UMAP versions
            umap_init = 'random' if n_components >= (n_samples - 1) else 'spectral'

            if n_components != requested_components or umap_init == 'random':
                print(
                    f"[UMAP] Adjusted n_components from {requested_components} to {n_components} (N={n_samples}); init='{umap_init}'.",
                    flush=True,
                )
            if safe_neighbors != requested_neighbors:
                print(
                    f"[UMAP] Adjusted n_neighbors from {requested_neighbors} to {safe_neighbors} (N={n_samples}).",
                    flush=True,
                )

            pc = umap.UMAP(
                    n_neighbors=safe_neighbors, n_components=n_components, init=umap_init, random_state=seed,
    ...
    def reduction_scatter(
        ...
        if continuous==True:
            ...
            sm = plt.cm.ScalarMappable(cmap=color, norm=norm)
            sm.set_array([])
            # Ensure colorbar attaches to the current Axes explicitly
            try:
                ax.figure.colorbar(sm, ax=ax)
            except Exception:
                fig_cb = ax.figure if hasattr(ax, 'figure') else plt.gcf()
                fig_cb.colorbar(sm)
...
def logVariance_allocation(df, labels, sampleName, replace_dict,
        dataType='Objective', model_path='/home/daweilin/StemCell/cancer_model.mat', save_root_path='./examples/analyze_demo/out/'):
    ...
    for col in stripdf.columns[:-1]:
        ...
        plt.tight_layout()
        # save the figures in configured output directory
        try:
            os.makedirs(save_root_path, exist_ok=True)
        except Exception:
            pass
        plt.savefig(os.path.join(save_root_path, f'{sampleName}_ShannonEntropy_{col}.png'))
...
def ObjectiveTradeOffs(
    ...
    if corrplot:
        ...
        sns.set_context("notebook", font_scale=2.)
        # Original legacy style: clustermap with dendrograms
