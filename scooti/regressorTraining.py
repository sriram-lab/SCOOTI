...
# Optional GPU-accelerated learners (PyTorch)
# Import lazily inside branches so torch is not a hard dependency
...
class regressorTraining:
    ...
    @staticmethod
    def get_unconstrained_models(root_path, norm, medium, file_suffix):
    ...
    @staticmethod
    def get_constrained_models(root_path, norm, kappa_arr, rho_arr, dkappa_arr, medium, stack_model, geneKO=False, geneList_path='', method='cfr', file_suffix='_fluxes.csv.gz'):
    ...
    def run_training(self, rank=False):
        ...
        self.unconstrained_models = self.get_unconstrained_models(
                self.unconstrained_models_path, self.uncon_norm, self.medium, self.file_suffix
                )
        ...
        if self.learner=='Lasso':
            # Import on demand to avoid requiring torch unless used
            try:
                from scooti.regressors.LassoTorch import LassoRegressor
            except Exception as e:
                raise RuntimeError(
                    "Lasso (GPU) learner requires PyTorch. Install via pip (e.g., "
                    "pip install --index-url https://download.pytorch.org/whl/cu118 torch==2.0.1+cu118) "
                    "or choose a non-torch learner (e.g., --learner L)."
                ) from e
            ...
            print('MLP...')
            # Initialize your wrapper (requires PyTorch)
            try:
                from scooti.regressors.MLPRegressor import TrainValWrapper
            except Exception as e:
                raise RuntimeError(
                    "MLP learner requires PyTorch. Install via pip (e.g., "
                    "pip install --index-url https://download.pytorch.org/whl/cu118 torch==2.0.1+cu118) "
                    "or choose a non-torch learner (e.g., --learner L)."
                ) from e
